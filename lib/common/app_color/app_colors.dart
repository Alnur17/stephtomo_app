import 'package:flutter/material.dart';

class AppColors{

  static const mainColor = Color(0xFF105481);
  static const bottomBackColor = Color(0xFFDBE7F7);
  static const bottomFadeColor = Color(0xFF9DB2CE);
  static const red = Color(0xfff44336);

  static const creamColor = Color(0xffA57758);
  static const creamColorLight = Color(0xfffaebe5);


  //fixed color
  static const transparent = Colors.transparent;
  static const greyLight = Color(0xffF5F5F5);
  static const grey = Color(0xffa5a5a5);
  static const greyDark = Color(0xff2A2A2A);
  static const silver = Color(0xffE8EBF0);
  static const white = Color(0xffffffff);
  static const black = Color(0xff000000);
  static const black100 = Color(0xff5C5C5C);
  static const green = Color(0xff00BA11);
  static const borderColor = Color(0xff000000);
  static const textColorWhite = Color(0xfffdfdfd);
  static const blue = Color(0xff1A63A6);
  static const lightBlue = Color(0xff40C4FF);
  static const hintColor = Color(0xff8b8989);
  static const greenLight = Color(0xffEDF8F7);
  static const whiteDark = Color(0xffBEBEBE);
  static const primary = Color(0xff4DB5AD);
  static const orange = Color(0xffff6200);
  static const failedColor = Color(0xffff3c00);


}